...
Every discovery, verifier, deduplicator, and editor subprocess uses Copilot JSONL output and `extract-agent-response.mjs`; rendered transcript text is never a machine response. Native dispatch may bypass extraction only for an explicit raw final-response field that is structurally separate from progress, tool, skill, reasoning, summary, and transcript data. Empty assistant messages are structural frames regardless of tool requests and remain subject to assistant-turn validation. A malformed `user.message` prompt echo is recoverable only when its stable metadata envelope proves Copilot's known stdout-redaction corruption; the extractor discards that non-authoritative prompt text and never repairs assistant output.
